  ...
  return (
    <form className="workout-form" onSubmit={(e) => { void handleSubmit(onSubmit)(e) }} noValidate>
      ...
      <Card className={cardClass}><CardContent className="p-0">
        <DesktopField id="workDuration" label={t('form.workLabel')} error={fieldError(errors.workDuration?.message, t)} registration={register('workDuration', { valueAsNumber: true })} />
        <DesktopField id="restDuration" label={t('form.restLabel')} error={fieldError(errors.restDuration?.message, t)} registration={register('restDuration', { valueAsNumber: true })} sep />
      </CardContent></Card>
      <Card className={cardClass}><CardContent className="p-0">
        <DesktopField id="intervals" label={t('form.intervalsLabel')} error={fieldError(errors.intervals?.message, t)} registration={register('intervals', { valueAsNumber: true })} />
        <DesktopField id="rounds" label={t('form.roundsLabel')} error={fieldError(errors.rounds?.message, t)} registration={register('rounds', { valueAsNumber: true })} sep />
      </CardContent></Card>
      <Card className={cardClass}><CardContent className="p-0">
        <DesktopField id="restBetweenRounds" label={t('form.blockRestLabel')} error={fieldError(errors.restBetweenRounds?.message, t)} registration={register('restBetweenRounds', { valueAsNumber: true })} />
      </CardContent></Card>
